...
module.exports = async function (opts) {
    ...
    if (userUrl && feedUrl){
        ...
        if (userNobot(user)){
            throw new Error("this user has #nobot in their profile. Add #yesMastofeed or #mastofeed to your profile if you want to use mastofeed and keep #nobot. NOTE: profiles are cached for up to 24 hours.")
        }
        isIndex = true;
...
async function itemsForFeed(opts,user,feed) {
    ...
    return items.filter((item)=>{
        return !!item && typeof item.object == 'object';// handle weird cases
    }).map((item)=>{

        var enclosures = (Array.isArray(item.object.attachment) ? item.object.attachment : []).filter((a)=>{
            return a.type == "Document" && typeof a.name == 'string' && typeof a.url == 'string';
        }).map((a)=>{
            return {
                name:a.name,
                type:a.mediaType || '',
